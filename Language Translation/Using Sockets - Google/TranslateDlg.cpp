...
BOOL CTranslateDlg::OnInitDialog()
{
   ...
   m_font.CreateFont(30, 0, 0, 0, FW_NORMAL, FALSE, FALSE, 0, DEFAULT_CHARSET, 0, 0, 0, 0, NULL);
   m_cTranslatedText.SetFont(&m_font,TRUE);

   m_cTranslatedText.SetWindowText(m_cTranslator.GetErrorMessage());
...
HBRUSH CTranslateDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
   ...
   if(pWnd->GetDlgCtrlID() == IDC_ERROR_MSG_TXT)
   {
